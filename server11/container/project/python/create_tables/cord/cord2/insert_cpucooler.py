...
class Product(Base):
    ...
    compatible_socket = Column(String)
...
class cpucoolermain:
    ...
    def insertcpucooler(item):
        item_list = {}
        item_list["cpucooler"] = item
        no = 1
        for i in item_list:
            for length in range(len(item_list[i])):
                ...
                product.maker_id = 110
                product.compatible_socket = item_list[i][int(length)]["対応ソケット"]
